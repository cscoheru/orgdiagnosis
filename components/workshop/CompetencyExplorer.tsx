...
export default function CompetencyExplorer({
  ...
  // 数据源切换：预计算 / 研讨会校准
  const [dataSource, setDataSource] = useState<'precomputed' | 'calibrated'>('precomputed');
  const [calibratedData, setCalibratedData] = useState<CompetencyTerm[] | null>(null);
  const [isLoadingCalibrated, setIsLoadingCalibrated] = useState(false);

  const switchToCalibrated = useCallback(async () => {
    if (dataSource === 'calibrated') return;
    setDataSource('calibrated');
    if (!calibratedData) {
      setIsLoadingCalibrated(true);
      try {
        const res = await fetch(`${API_BASE}/api/v1/competency/calibrated`);
        const json = await res.json();
        if (json.success && json.data) {
          setCalibratedData(json.data);
        }
      } catch (err) {
        console.error('Failed to load calibrated data:', err);
      } finally {
        setIsLoadingCalibrated(false);
      }
    }
  }, [dataSource, calibratedData]);

  // 当前展示数据
  const displayCompetencies = dataSource === 'calibrated' && calibratedData
    ? calibratedData
    : competencies;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1.5 items-center">
        <button
          onClick={() => setDataSource('precomputed')}
        ...
        >
          ...
          <span className={`ml-1.5 px-1.5 py-0.5 rounded text-[10px] font-semibold ${
            dataSource === 'precomputed' ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400'
          }`}>
            {competencies.length}
          </span>
        </button>
        <button
          onClick={switchToCalibrated}
          disabled={isLoadingCalibrated}
          className={`px-3 py-1 rounded-lg text-xs font-medium transition-all flex items-center gap-1 ${
            dataSource === 'calibrated'
              ? 'bg-white text-violet-700 shadow-sm border border-violet-200'
              : 'text-gray-500 border border-gray-200 hover:bg-gray-50 disabled:opacity-50'
          }`}
        >
          研讨会校准
          {isLoadingCalibrated && (
          ...
          {calibratedData && (
            <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${
              dataSource === 'calibrated' ? 'bg-violet-100 text-violet-600' : 'bg-gray-100 text-gray-400'
            }`}>
              {calibratedData.length}
            </span>
          )}
        </button>
      </div>
