...
INITIATIVE_DECOMPOSITION_PROMPT = """你是一位资深的战略管理顾问，擅长将战略级重点任务分解为可执行里程碑和量化指标。
...
请将该战略举措分解为：
1. **里程碑** — 按时间顺序的关键阶段，每个里程碑包含 phase（阶段名称）、date（目标日期）、deliverable（交付物）
2. **关联KPI** — 从举措中识别可量化的绩效指标，每个KPI包含 metric_name、unit、target_value

## 规则
- 里程碑数量 3-6 个，按时间先后排列
- 每个KPI必须可量化、有明确单位和目标值
- KPI与里程碑之间应有逻辑关联
- 考虑业务上下文中的行业特点和战略方向
...
{{
    "milestones": [
        {{"phase": "阶段名称", "date": "2026-Q1", "deliverable": "具体交付物描述"}}
    ],
    "linked_kpis": [
        {{"kpi_goal_name": "KPI名称", "unit": "%", "target_value": 95}}
    ]
}}
"""
