-- ============================================================
-- DeepConsult Project Management System (Simplified)
-- Created: 2026-03-21
...
CREATE INDEX IF NOT EXISTS idx_requirements_saved_at ON project_requirements(last_saved_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_outlines_version ON project_outlines(project_id, version DESC);
...
CREATE INDEX IF NOT EXISTS idx_slides_section ON project_slides(project_id, section_id);
...
CREATE POLICY "Users can view own requirements"
    ...
    USING (project_id IN (SELECT id FROM projects WHERE created_by = auth.uid() OR created_by IS NULL));
...
CREATE POLICY "Service role can modify slide models"
...
DROP TRIGGER IF EXISTS update_slide_models_updated_at ON slide_models;
